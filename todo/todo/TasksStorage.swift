...
class TasksStorage {
    ...
    private func fileURL() throws -> URL? {
         FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: "group.ru.azharkova.todo.TodoWidget")?
    ...
    func save(scrums: [TasksList], completion: @escaping (Result<Int, Error>)->Void) {
        DispatchQueue.global(qos: .background).async {
            do {
                ...
                guard let outfile = try? self.fileURL() else {return}
